...
class ConsensusEngine:
    ...
    def ensure_schema(self) -> None:
        with self._graph.cursor() as cur:
            ...
            """)
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_learning_status ON entity_learning(status)"
            )

    def cross_validate(
        self, detections: list[dict], available_modes: list[str] | None = None
        ...
        verdicts = []
        for name, g in grouped.items():
            score = sum(MODE_WEIGHTS.get(m, 0.5) for m in g["found_by"]) / max(
                total_weight, 0.01
            )
    ...
    def learn_from_graph(
        self, sample_count: int = 200, dry_run: bool = True
    ) -> ConsensusResult:
    ...
    def filter_rejected(self, entities: list[dict]) -> list[dict]:
        ...
        with self._graph.cursor() as cur:
            cur.execute(
                "SELECT entity_name FROM entity_learning WHERE status = 'rejected'"
            )
